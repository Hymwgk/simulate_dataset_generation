...
**例子1：**已知相机坐标系$C$下各点$p$相对于相机的位置$^{C}p$，以及相机坐标系相对与机器人基座坐标系的变换$^{B}T_C$，求解各点相对于机器人基座坐标系的点坐标$^{B}p$

<img src="点云与夹爪坐标变换.assets/image-20210831210923247.png" alt="image-20210831210923247" style="zoom: 33%;" />

答案：
$$
^{B}p= ^{B}T_C\cdot^{C}p= \begin{bmatrix} R&t\\0&1\end{bmatrix}\cdot^{C}p=R\cdot^{G}p+t
$$
其中，$R,t$ 分别是从坐标系$B$ 到$C$ 的旋转矩阵以及 平移向量。



**例2**：已知各个点$p$在基座坐标系$B$下的点坐标$^{B}p$ ，以及相机坐标系相对与机器人基座坐标系的变换$^{B}T_C$，求解各个点相对于相机坐标系的点坐标$^{C}p$
